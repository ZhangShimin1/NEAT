    ...
    def __init__(
        ...
        if bn == "bn":
            self.bns = nn.ModuleList(
                [BatchNorm1d(hidden_size[l]) for l in range(num_layers)]
            )
        elif bn == "ln":
            self.bns = nn.ModuleList(
                [nn.LayerNorm(hidden_size[l]) for l in range(num_layers)]
            )
        elif bn == "tdbn":
            self.bns = nn.ModuleList(
                [
                    ThresholdDependentBatchNorm1d(
                        alpha=1.0,
                        v_th=spiking_neuron().threshold,
                        num_features=hidden_size[l],
                    )
            ...
            self.bns = nn.ModuleList(
                [
                    TemporalEffectiveBatchNorm1d(
                        T=spiking_neuron().time_step, num_features=hidden_size[l]
                    )
                    for l in range(num_layers)
                ]
            )
            ...
            print(
                f"{bn} not implemented, BatchNorm type must be one of bn, ln, tdbn, tebn"
            )
            raise NotImplementedError
        ...
        for hidden_layer_i in range(num_layers):
            ...
            if hidden_layer_i == (num_layers - 1):
                exec(
                    "self.spk"
                    + str(hidden_layer_i)
                    + " = spiking_neuron(neuron_num=hidden_size, recurrent=False)"
                )
            else:
                exec(
                    "self.spk"
                    + str(hidden_layer_i)
                    + " = spiking_neuron(neuron_num=hidden_size)"
                )
        ...
        if spiking_neuron_name == "celif":
            ...
            for hidden_layer_i in range(num_layers):
                exec(
                    "self.spk"
                    + str(hidden_layer_i)
                    + " .TE = self.celif_TE".format(hidden_layer_i)
                )
