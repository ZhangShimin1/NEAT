...
class EnergyCalculator:
    ...
    def adlif(self, f_in, f_out, dim_in, dim_out, rec):
        energy = (
            dim_in * dim_out * f_in + 2 * dim_out * f_out
        ) * self.eac + 5 * dim_out * self.emac
        if rec:
            energy = energy + dim_in * dim_in * f_in * self.eac
    ...
    def celif(self, f_in, f_out, dim_in, dim_out, rec):
        energy = (
            dim_in * dim_out * f_in + dim_out * f_out
        ) * self.eac + 3 * dim_out * self.emac
        if rec:
            energy = energy + dim_in * dim_in * f_in * self.eac
    ...
    def ltc(self, f_in, f_out, dim_in, dim_out, rec):
        energy = (
            dim_in * dim_out * f_in + 3 * dim_out * f_out
        ) * self.eac + 4 * dim_out * (1 + dim_in) * self.emac
        if rec:
            energy = energy + dim_in * dim_in * f_in * self.eac
    ...
    def glif(self, f_in, f_out, dim_in, dim_out, rec):
        energy = (
            dim_in * dim_out * f_in * self.eac
            + (
                2 * dim_out**3
                + 3 * dim_out**2
                + f_out * (2 * dim_out**2 + dim_out**3 + dim_out**4 + dim_out)
            )
            * self.emac
        )
        if rec:
    ...
    def clif(self, f_in, f_out, dim_in, dim_out, rec):
        energy = (
            dim_in * dim_out * f_in * self.eac
            + (dim_out * f_out + dim_out + dim_out**2) * self.emac
        )
        if rec:
            energy = energy + dim_in * dim_in * f_in * self.eac
    ...
    def calculate(self):
        ...
        if self.dataset_name == "shd":
            ...
            float_energy += self.float_readin(
                dim_in=self.readin_dim, dim_out=self.neuron_nums[0]
            )

        event_energy += self.intermediate_snn_layer_energy()

        float_energy += self.readout(
            f_in=self.firing_rates[-1],
            dim_in=self.neuron_nums[-1],
            dim_out=self.readout_dim,
        )
    ...
    def intermediate_snn_layer_energy(self):
        ...
        for i in range(len(self.neuron_nums) - 1):
            ...
            if isinstance(self.module_type, list):
                neuron = (
                    self.module_type[i]
                    if i < len(self.module_type)
                    else self.module_type[-1]
                )
            else:
            ...
            if isinstance(self.recurrent_flag, list):
                rec = (
                    self.recurrent_flag[i]
                    if i < len(self.recurrent_flag)
                    else self.recurrent_flag[-1]
                )
            else:
