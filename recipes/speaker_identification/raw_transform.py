        ...
        if signal.shape[1] < self.size:
            ...
            if self.mode == "constant":
                signal = torch.nn.functional.pad(
                    signal, pad_width[1], "constant", value=signal.min()
                )
...
class RandomReverb:
    ...
    def __call__(self, x):
        ...
        x = (
            augment.EffectChain()
            .reverb(reverberance, damping, room_scale)
            .channels(1)
            .apply(x, src_info=self.src_info)
        )
        t1 = time.time()
        return x
...
class BackgroundNoiseGenerator:
    def __init__(
        ...
        self.tfs = Compose([PadToSize(num_samples, "wrap"), RandomCrop(num_samples)])
        if self.in_memory:
...
class RandomGain:
    def __init__(self, min_gain_in_db=-18.0, max_gain_in_db=6.0, prob=0.5, sr=16000):
        self.gain = torch_audiomentations.Gain(
...
class PeakNormalization:
    def __init__(self, sr=16000):
        self.peak_norm = torch_audiomentations.PeakNormalization(
            apply_to="only_too_loud_sounds", p=1.0, sample_rate=sr
        )
...
def get_raw_transforms_v2(
    ...
    if train:
        ...
        if background_noise_path:
            noise_gen = BackgroundNoiseGenerator(
...
def train_transforms(is_train, size, sample_rate=16000):
    if is_train:
        tfs = [
            OneOf(
                [PadToSize(size, mode="wrap"), PadToSize(size, mode="constant")],
                p=[0.5, 1 - 0.5],
            ),
            RandomCrop(size),
            UseWithProb(RandomGain(sr=sample_rate), prob=0.5),
...
def test_transforms(sample_rate):
    return Compose(
        [
