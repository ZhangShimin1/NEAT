...
def parse_prob_logs(exp_dir, key="accuracy", output="output"):
    ...
    if not train_objf:
        raise KaldiLogParseException(
            "Could not find any lines with {k} in  {l}".format(
                k=key, l=train_prob_files
            )
        )
    ...
    if not iters:
        raise KaldiLogParseException(
            "Could not any common iterations with key {k} in both {tl} and {vl}".format(
