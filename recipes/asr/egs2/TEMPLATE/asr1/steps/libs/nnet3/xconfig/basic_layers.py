...
class XconfigLayerBase(object):
    ...
    def convert_to_descriptor(self, descriptor_string, all_layers):
        ...
        if pos != len(tokens) - 1:
            raise RuntimeError(
                "Parsing Descriptor, saw junk at end: {0}".format(
                    " ".join(tokens[pos:-1])
                )
            )
        return descriptor
...
class XconfigTrivialOutputLayer(XconfigLayerBase):
    ...
    def check_configs(self):
        if (
            ...
            raise RuntimeError(
                "In output, objective-type has invalid value {0}".format(
...
class XconfigOutputLayer(XconfigLayerBase):
    ...
    def check_configs(self):
        if self.config["dim"] <= -1:
            raise RuntimeError(
                "In output-layer, dim has invalid value {0}".format(self.config["dim"])
            )
        ...
        if (
            ...
            raise RuntimeError(
                "In output-layer, objective-type has invalid value {0}".format(
                    self.config["objective-type"]
                )
            )
...
class XconfigExistingLayer(XconfigLayerBase):
    ...
    def check_configs(self):
        if self.config["dim"] <= 0:
            raise RuntimeError(
                "Dimension of existing-layer '{0}'should be positive.".format(self.name)
            )
