...
class Trainer(BaseTrainer):
    def __init__(self, *args, cal_FR=True, **kwargs):
        ...
        self.dataset_name = (
            kwargs.get("args", None).data.dataset
            if hasattr(kwargs.get("args", None), "data")
            else None
        )
        if cal_FR:
    ...
    def evaluation_loop(self, description: str, gather_step_output: bool = False):
        ...
        for dl_idx, (dl_id, dataloader) in enumerate(self.eval_dataloaders.items()):
            ...
            if self.cal_FR:
                ...
                json_path = (
                    self.metrics_dir
                    / f"energy_consumption_epoch_{self.state.epochs_trained}.json"
                )
    ...
    def get_read_layer_info(self):
        ...
        if not linear_layers:
            logger.warning(
                "No Linear layers found in the model. Returning None for dimensions."
            )
            return None, None
        ...
        logger.info(
            f"Found first Linear layer: {first_linear_name}, last Linear layer: {last_linear_name}"
        )
